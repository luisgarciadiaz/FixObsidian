...
from core.vortexy_resolver import resolve_author_title, parse_author_title_from_filename
from core.vortexy_db import load_author_genre_map
from core.vortexy_enricher import MetadataEnricher
from core.vortexy_stats import run_stats_only
from tools.consolidate_tracks import consolidate_tracks
from tools.merge_duplicates import merge_extension_duplicates, merge_fuzzy_duplicates


def fix_notes(vault_path, library_path, dry_run, limit, start_at, organize, cfg, enrich=True, clear_cache=False, stats_only=False):
    if stats_only:
        run_stats_only(vault_path, cfg=cfg, limit=limit, start_at=start_at)
        return
    ...
    read_status = cfg.get("read_status", "to-read")
    ...
    merged_fuzzy = merge_fuzzy_duplicates(vault_path, dry_run=dry_run)
    if merged_fuzzy:
        print(f"   Merged {merged_fuzzy} fuzzy-duplicate note(s)")
        print()
    ...
    def process_one(global_idx, fname):
        ...
        if author != "Unknown Author" and title:
            ...
            title_clean = PAREN_RE.sub('', title_lower).strip()
            author_clean = PAREN_RE.sub('', author_lower).strip()
            if title_clean in author_genre_map and author_clean not in author_genre_map:
                author = PAREN_RE.sub('', title).strip()
                title = author_lower.strip()
        ...
        if enricher and (existing_isbn or author != "Unknown Author"):
            ...
            if not enriched.get("suggested_category"):
                try_author = CLEAN_RE.sub(' ', title).strip()
                try_title = CLEAN_RE.sub(' ', author).strip()
                if try_author.lower() != author.lower():
                    swapped = enricher.enrich(existing_isbn, try_title, try_author, year=bracket_year)
                    if swapped.get("suggested_category") or swapped.get("publisher") or swapped.get("publish_date") or swapped.get("synopsis"):
                        author, title = try_author, try_title
                        enriched = swapped
        ...
        if organize:
            ...
            if not db_subfolder and ", " in author_key:
                parts = author_key.split(", ", 1)
                reversed_key = f"{parts[1]} {parts[0]}"
                db_subfolder = author_genre_map.get(reversed_key)
            if not db_subfolder and " _ " in author_key:
                for part in author_key.split(" _ "):
                    part = part.strip().strip(",")
                    sf = author_genre_map.get(part)
                    if sf and sf in valid_folders:
                        db_subfolder = sf
                        break
                    if ", " in part:
                        last, first = part.split(", ", 1)
                        reversed_part = f"{first} {last}"
                        sf = author_genre_map.get(reversed_part)
                        if sf and sf in valid_folders:
                            db_subfolder = sf
                            break
            enrich_sf = enriched.get("suggested_category", "")
            if db_subfolder and db_subfolder in valid_folders:
                target_subfolder = db_subfolder
            elif enrich_sf and enrich_sf in valid_folders:
        ...
        discovered_isbn = enriched.get("discovered_isbn", "")
        cover_url = enriched.get("cover_url", "")
        use_isbn = existing_isbn or discovered_isbn

        new_content = make_obsidian_content(
            author=author, title=title, category=use_category,
            isbn=use_isbn, original_name=original_name,
            ...
            synopsis=synopsis, cover_url=cover_url, read_status=read_status
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=0, help="Process only N notes")
    parser.add_argument("--start-at", type=int, default=0, help="Skip first N notes")
    ...
    parser.add_argument("--organize", action="store_true", default=cfg.get("organize", True), help="Move notes into subfolders")
    parser.add_argument("--no-organize", action="store_false", dest="organize", help="Don't move notes")
    parser.add_argument("--enrich", action="store_true", default=True, help="Fetch metadata from Open Library API")
    parser.add_argument("--no-enrich", action="store_false", dest="enrich", help="Disable enrichment")
    parser.add_argument("--clear-cache", action="store_true", default=False, help="Clear metadata cache")
    parser.add_argument("--stats-only", action="store_true", default=False, help="Scan vault and print statistics without modifying files")
    args = parser.parse_args()
    fix_notes(args.vault, args.library, args.dry_run, args.limit, args.start_at, args.organize, cfg, enrich=args.enrich, clear_cache=args.clear_cache, stats_only=args.stats_only)
